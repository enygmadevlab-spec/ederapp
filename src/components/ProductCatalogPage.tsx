...
interface ProductCatalogPageProps {
  ...
  description: string;
  eyebrow: string;
  fallbackProducts: ServiceProduct[];
  cartNounSingular: string;
  cartNounPlural: string;
}
...
export function ProductCatalogPage({
  ...
  const [services, setServices] = useState<ServiceProduct[]>(fallbackProducts);
  const [loading, setLoading] = useState(true);
  const isLight = theme === 'light';
  const segmentCartItems = cart.filter(
    (item) => (item.businessSegment || 'nautica') === segment
  );
  ...
  useEffect(() => {
    ...
    try {
      ...
      const unsubscribe = onSnapshot(
        query(collection(db, BUSINESS_COLLECTIONS[segment])),
        (snapshot) => {
          const dbServices: ServiceProduct[] = snapshot.docs.map((document) =>
            normalizeServiceProduct(
          ...
          setServices(dbServices.length > 0 ? dbServices : fallbackProducts);
          setLoading(false);
        },
        (error) => {
          console.error(`Erro ao carregar catálogo de ${segment}:`, error);
          setServices(fallbackProducts);
          setLoading(false);
        }
      );
  ...
  const handleAdd = (service: ServiceProduct) => {
    ...
    setTimeout(
      () => setAddedIds((prev) => prev.filter((id) => id !== `${segment}:${service.id}`)),
      2000
    );
  };
    ...
    <div className="min-h-screen py-12 animate-fade-in" style={{ backgroundColor: pageBackground }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-16 text-center">
          <span className="text-xs font-bold tracking-[0.3em] uppercase mb-3 block" style={{ color: accentSoftColor }}>
            {eyebrow}
          </span>
          <h1 className="text-5xl md:text-6xl font-black mb-6" style={{ color: titleColor }}>
            {title}
          </h1>
              ...
              return (
                ...
                >
                  ...
                  <div className="p-6 flex-grow flex flex-col">
                    <h3 className={`text-xl font-bold mb-2 leading-tight transition-colors ${isLight ? 'group-hover:text-sky-700' : 'group-hover:text-sky-300'}`} style={{ color: titleColor }}>
                      {service.title}
                    </h3>
                    <p className="text-sm mb-6 flex-grow line-clamp-3" style={{ color: bodyColor }}>
                      {service.description}
                    </p>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex-1">
                        <p className="text-xs uppercase font-semibold mb-1" style={{ color: subtleColor }}>
                          Investimento
                        </p>
        ...
        {segmentCartItems.length > 0 && (
          <div className="bg-gradient-to-r from-sky-600/20 to-cyan-600/20 border-2 border-sky-500/50 rounded-2xl p-8 text-center mt-12" style={{ background: ctaBackground }}>
            <h2 className="text-2xl md:text-3xl font-bold mb-3" style={{ color: titleColor }}>
              Você tem {segmentCartItems.length} {segmentCartItems.length === 1 ? cartNounSingular : cartNounPlural} no carrinho
            </h2>
            <p className="mb-6" style={{ color: bodyColor }}>
              Total: <span className="font-black text-2xl" style={{ color: accentColor }}>R$ {segmentCartItems.reduce((acc, item) => acc + item.price, 0).toFixed(2)}</span>
            </p>
            <Link
              href="/checkout"
